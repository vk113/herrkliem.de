<script>
	...
	function onTouchMove(e){
		if (resize) {
			...
			l = e.touches[0].clientX - X;
			t = e.touches[0].clientY - Y;
		}
	}
...
<div class="draggable {in_front?"z-5":"pointer-events-none"}" style="top:{t}px; left:{l}px; width:{w}px; height:{h}px" bind:offsetWidth={w} bind:offsetHeight={h}>
	...
	{/if}
	<div on:mousedown={onMouseDown} on:touchstart={onTouchStart} on:touchmove|preventDefault={onTouchMove} >
			<slot name="content"></slot>
		{#if resizable}
			{#if w_constant}
				<div class="absolute bottom-[1px] left-[1px] w-4 h-4 bg-white cursor-ns-resize" on:mousedown={onMouseResize} on:touchstart={onMouseResize}>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
					</svg>
				</div>
			{:else}
				<div class="absolute bottom-0 right-0 w-5 h-5 bg-white z-2 cursor-nwse-resize" on:mousedown={onMouseResize} on:touchstart={onMouseResize}>
