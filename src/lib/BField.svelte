...
<Movable bind:t={b_field.t} bind:l={b_field.l} bind:w={b_field.w} bind:h={b_field.h} resizable in_front={!$e_in_front}>
    ...
    <div slot="content">
        <div class="text-red-500 mx-1">B<sub>{i+1}</sub></div>
        <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full flex flex-col justify-evenly  border-red-500 border-2">
            {#each rows as row}
            <div class="flex flex-row justify-evenly opacity-50">
                {#each circles as circle}
                <div class="rounded-full border-2 border-red-500 p-2 w-8 h-8 flex ">
                    {#if direction_down}
                    <div class="h-0 w-0 border-2 border-red-500 rounded-full m-auto self-center">
